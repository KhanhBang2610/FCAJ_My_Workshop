...
### 1. Tổng quan dự án
**Smart Media Analytics (SMA)** là hệ thống local-first tự động nạp, tách cảnh, phiên âm và tìm kiếm ngữ nghĩa media. Người dùng chỉ cần gõ yêu cầu tự nhiên (VD: “hoàng hôn trên biển”) để tìm đúng timestamp video, tiết kiệm thời gian xem thủ công. Ban đầu, hệ thống chạy 100% nội bộ qua Docker giúp bảo mật và tối ưu chi phí thử nghiệm, sau đó có thể dễ dàng nâng cấp cấu trúc tương ứng lên AWS (S3, Bedrock, RDS) khi cần.
...
- **Mạng & Phân phối:** Amazon Route 53 (DNS), AWS Amplify (Frontend), Amazon VPC (kèm NAT Gateway).
- **Bảo mật:** Amazon Cognito (Xác thực JWT), AWS Secrets Manager (Quản lý keys).
- **Lưu trữ & Database:** Amazon S3 (Lưu trữ Media), Amazon RDS PostgreSQL (Metadata), Amazon ElastiCache (Redis Cache).
- **Tính toán & API:** AWS App Runner (FastAPI Backend), Amazon API Gateway (Routing), Amazon ECS Fargate (Xử lý video).
- **AI/ML:** Amazon Bedrock (Embeddings), Amazon Transcribe (Speech-to-Text).
- **Điều phối & Tích hợp:** Amazon SQS, EventBridge (Hàng đợi & Event), AWS Step Functions (Điều phối Workflow).
- **DevOps & Giám sát:** Amazon ECR (Docker Registry), Amazon CloudWatch & AWS X-Ray (Logs & Tracing).
...
### 8. Kết quả kỳ vọng
- **Cải tiến kỹ thuật:** SMA cho phép người dùng nạp media, tự động tạo chỉ mục ngữ nghĩa, tìm kiếm bằng ngôn ngữ tự nhiên và nhảy trực tiếp tới đúng timestamp.
- **Giá trị dài hạn:** Phù hợp cho nhóm thiết kế, dựng phim cần quản lý media nội bộ an toàn và hiệu quả. Kiến trúc local-first giúp phát triển nhanh, kèm đường nâng cấp rõ ràng lên AWS khi cần.
