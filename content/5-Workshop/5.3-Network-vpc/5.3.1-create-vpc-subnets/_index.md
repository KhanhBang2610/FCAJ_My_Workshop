...
Để đảm bảo tính sẵn sàng cao (High Availability) và bảo mật tuyệt đối cho các Microservices, tiến hành tự thiết kế một Virtual Private Cloud (VPC) riêng. Tiến hành tuân thủ nguyên tắc tách biệt hoàn toàn các tài nguyên giao tiếp với bên ngoài (như Application Load Balancer) khỏi các dịch vụ xử lý nội bộ và cơ sở dữ liệu (ECS, RDS, Redis).

Thay vì phải tạo thủ công từng thành phần rời rạc, tiến hành sử dụng công cụ **VPC and more** để khởi tạo nhanh toàn bộ kiến trúc mạng đa tầng, trải dài trên 2 Availability Zones với định tuyến (routing) đã được cấu hình sẵn.
