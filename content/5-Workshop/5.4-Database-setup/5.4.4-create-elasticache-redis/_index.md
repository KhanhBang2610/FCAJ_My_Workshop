...
Tương tự như RDS, tiến hành đặt cụm Redis này vào vùng Private Subnet để đảm bảo tuân thủ kiến trúc bảo mật Zero-Trust.
...
7. **Subnet ID:** *Lưu ý quan trọng:* Hệ thống có thể sẽ tự động chọn cả 4 subnets. Hãy bấm nút **Manage**, **bỏ chọn 2 Public Subnets**, chỉ giữ lại dấu tích ở 2 Private Subnets của dự án.
8. Bấm **Create**.
...
Vì giao diện AWS ElastiCache mới sử dụng dạng trình tự từng bước, hãy thực hiện cấu hình như sau:
...
3. **Step 2: Advanced settings (Cài đặt nâng cao - Rất quan trọng):**
   * **Security groups:** Bấm nút **Manage** → Bỏ chọn nhóm `default`, tích chọn duy nhất nhóm **`cloudforge-db-redis-sg`** → Bấm **Save**. *(Lỗi kinh điển: Quên gắn Security Group khiến ECS không thể kết nối tới Redis)*.
   * **Encryption in transit:** Mặc định AWS có thể sẽ Bật (Enable) tính năng này. Nếu bật, bạn bắt buộc phải cấu hình URL kết nối là `rediss://` thay vì `redis://` trong ứng dụng của hệ thống. Nếu không, ứng dụng sẽ bị treo (Timeout) khi kết nối vì server đang đợi TLS handshake.
...
Sau khi trạng thái của cụm Redis chuyển sang **Available**, hãy click vào tên `cloudforge-redis`.
...
**Bước tiếp theo:** Hệ thống dữ liệu lõi đã hoàn tất. Tiến hành tiến tới phần [**5.4.5**](../5.4.5-test-connection/) để tổng hợp lại các Endpoint cần thiết, chuẩn bị hành trang đưa mã nguồn Backend lên Cloud.
